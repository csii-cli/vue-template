...
module.exports = {
  ...
  chainWebpack: (config) => {
    ...
    // 设置别名
    config.resolve.alias.set('@api', resolve('src/api')).set('@views', resolve('src/views')).set('public', resolve('public')).set('@', resolve('src'))
    // 判断环境加入模拟数据
    const entry = config.entry('app')
    if (process.env.VUE_APP_BUILD_MODE !== 'NOMOCK') {
      entry.add('@/mock').end()
    }
    config.output.filename(`js/[name].js?version=${Version}`).chunkFilename(`js/[name]_chunk.js?version=${Version}`).end()
  }
}
