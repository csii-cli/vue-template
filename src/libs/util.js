  ...
  appList.map((app) => {
    if (app.path && app.path !== '') {
      ...
      routes.push({
        path: realPath,
        component: (r) =>
          require.ensure([], () => r(require(`@/views/demo${realPath}/index.vue`)), 'base'),
        name: app.path,
        meta: {
          id: app.id,
          name: app.name
...
// pkcs7填充方法
export const pkcs7Padding = (inputBytes) => {
  if (!inputBytes instanceof Array) {
    throw 'The parameter `inputBytes` must be bytes array.'
  }
  if (inputBytes == null || inputBytes.length <= 0) {
    throw 'The parameter `inputBytes` must not be null or empty.'
  }
  const needToAddCount = 16 - (inputBytes.length % 16)
  const result = new Array()
  for (const j in inputBytes) {
    result.push(inputBytes[j])
  }
  for (let i = 0; i < needToAddCount; i++) {
    result.push(needToAddCount)
  }
  return result
}
/* 读取文件 */
